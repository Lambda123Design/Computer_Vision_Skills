...
Hello everyone. Welcome to the video of GoogLeNet architecture. So in this particular video we will be discussing about GoogLeNet. The other name is inception v1 also okay which was introduced by the Google team okay. And it was one of the winners in the ImageNet competition. It was developed by the researchers at Google and significantly outperformed various architectures like AlexNet and VGG. Net by reducing the computational cost while improving the accuracy. Okay, now, uh, from here, I hope you get a high level idea that the journey of AlexNet, then VGG, and then finally coming to GoogLeNet or Inception that we are talking right now. Okay. So here we are telling that, okay, that's not enough. Okay. So that's not enough. We need to go deeper. Okay. The idea is that in AlexNet eight layers in VGG, then 16 layers or 19 layers Okay. So slowly, slowly we'll be increasing the number of layers. So here also we'll be increasing the depth as well as the breadth. Okay. It means that I will be. So previously you have been adding blocks in this way. Okay. So sequentially one after another. Okay. But this time we will be increasing the width of the network also. Okay. So in a similar pattern we'll be using multiple convolution blocks parallelly. Okay. So that's why it's a very famous one. That's not enough. We need to go deeper. Okay. So deeper with respect to the width and as well as the depth of the network. Okay. So the things that we'll be discussing is the network architecture. Then coming to the inception module, the one cross one convolutions which has been used referred to as pointwise convolutions also. Then coming to the global average pooling okay. Layer. So this is basically an alternative to dense or fully connected layer which has been used in this architecture. Then we'll be talking about auxiliary classifiers and finally coming to different variants of, uh, inception. Okay. So let's get started.

So the inception module when I am talking about. Okay. So this module has multiple type of convolution blocks okay. So let me just show you this part okay. So right now as you can see. So this is basically the architecture okay. So this is the architecture that I have okay. And this is basically the inception module okay. That will be talking about right now as you can see. So this particular architecture is very very long. Okay. The idea to fit in this particular uh notebook it was tough. But uh, let's go through this okay. So because in the official research paper it is available. So right now. So this is basically the research paper of inception okay. That I've opened right now okay. And from here let's jump into the architecture part okay. So the architecture starts from the bottom. You can see GoogLeNet network with all the bells and whistles. So let me just zoom into so that you can understand actually what is going on here. So as you can see. So there is basically the input image that I have. Then you have the convolution layer okay. Then coming to max pooling layer. And here one layer which has been added which is the local response normalization the layer. If you remember we discussed it in the AlexNet video. Now here after that again some convolutions okay. And now coming to this particular block that I have okay. Now this block is important okay. Now this where you can see parallelly multiple operations are going on okay. Now this particular block is actually referred to as the inception block because you will see a repetitive module which will be used. Okay, so if I am talking with respect to this module, so let's try to understand the inception module.

So right here we have our our previous layer. So whatever the outputs that are coming from the previous layer okay. Now it has two different versions the naive version and the dimension reduction version okay. So right now here one cross one convolution okay. Then comes your uh three cross three okay. Then comes your five cross five three cross three max pooling. And finally we have our filter concatenation. It means that all the output from the different blocks will be concatenated together okay. So if you look into the naive version okay. And the dimension reduction version. So here you will see that one cross one convolutions have been kept okay. So this is something new. Okay. So here the term has been used dimension reduction. Okay. So the idea is that by using one cross one you will perform dimensionality reduction. Okay. And even the computational cost also. So they have kept it. The idea is that most of the times we have gone through the implementation of like a three cross three, okay, like 64 filters that we have used. Again, we have used a three cross three filter. Okay. So this is three cross three is basically the filter size. And then it comes to the number of filters that you have used. So 64 128. So slowly slowly like 256 okay. But the idea is that if you want to, uh, basically reduce the dimension, then what you have to do. Okay. So the idea that we will be taking into. Okay. So from three cross 364 that I have okay I can if I want to perform a reduction. So I will directly use something like this. Okay. And then again I will use something towards the higher right. Okay. So by using this one cross one you can actually reduce it. So for example you can even reduce here also. So here from 64 previously which has been used we will get down to 32. So simply like you can reduce the number of channels also as well as increase it also. It totally depends upon the use case. But here in this particular architecture. Okay. They have used one cross one which is also referred to as pointwise convolution. Okay. Now pointwise convolution they have used it okay for dimensionality reduction. And the idea is that whenever you are performing a feature extraction task okay. So you have to understand. So right now if you look here okay so like three cross three is responsible for feature extraction. Five cross five is responsible for feature extraction. So in the dimensionality reduction. So whenever the feature extraction takes place before that they are actually reducing the dimension by using a one cross one convolution and with respect to the max pooling. Because in max pooling there is no feature extraction okay directly reduces the dimension of the feature maps. Okay. But here after max pooling they have added a one cross one convolution for dimensionality reduction okay. So when I'm talking about one cross one, we know that one cross one cannot extract any feature. This should be important because one cross one is simply think about it's just a single uh, pixel. If I'm talking about okay or a single unit, okay in such a big feature map. But you need multiple cells. If you look into a single cell, you won't get the pattern. But if you look into ten pixels at the same time, okay, you will get some type of pattern. Okay. So that is basically the idea. And based on this. So you have the inception module. Now this inception module will be repeated multiple times. Okay. So when I'm talking about multiple times so the numbers are also given okay with respect to this. So here you can see like uh okay so 3A3B4ABCDE okay. So multiple inception blocks have this mean that I'll be directly taking this block and I will be implementing it okay. It means that I'll be just picking up this block. And then I will try to replicate the same block multiple times okay. But there can be some type of minor changes with respect to the number of filters that we are using. Because here if you look here okay. So yes with respect to filter changes. So this is your one cross one filter with respect to three. Cross three and reduce. It means that. So whenever here the reduce version is written. So this basically means that you are performing a one cross one with three cross three. Okay. Then again coming to five cross five. Very similar meaning. It means that with the five cross five you are using the one cross one point wise convolution. So simple convolutions with respect to this also okay. And finally you can look into okay. So a lot of parameters okay. So a lot of parameters are there. So this model took a lot of time for training for sure. Okay.

Now let's get back to the paper. Okay. So right now in this particular paper. So this is basically the block that we were talking about okay. So like your one cross one here, one cross one here. And finally one cross one here. So after max pooling so before the feature extraction part. So there has been a dimensionality reduction okay. Now what about the filter concatenation or depth concatenation that we can refer into okay. So that is taking place now this particular block we are repeating. So one then comes two times then comes three times okay. So after three times, so there has been some change in the network. Okay. Now when I'm talking with respect to the changes. So now this inception module is pretty much the same. But from here another pathway has gone okay towards the auxiliary classifier. Now this particular classifier. So most of the times till now you have learned that the classification in any type of image classification networks okay. So there is a single softmax at the last. But here in between there are softmax layers also okay. So similarly again you can see multiple times the inception module has been uh like replicated okay. So you can see again this block comes again. This block comes. And finally you have your second auxiliary classifier which is basically the softmax one. And again uh, finally the inception blocks are repeating okay. And finally you have an average pool. So this is referred to as the global average pooling layer. So this is basically the alternate of FCS. The idea is to use less fully connected layers okay. Because fully connected layers increases the number of parameters. Let's get back to the notebook. Okay. So as we were discussing about, uh, the inception okay. So right now if you look into so the entire network calculation which has been given okay. And from there you can easily understand that, uh, which layer has been used, how many times the inception block has been used? Okay. Now let's try to break the block and just try to understand like what is going on. Okay.

So the structure of inception. So the first thing that comes is basically the new one, the one cross, one convolution. Okay. Then comes your three cross three convolution. Then you have your five cross five convolution. Okay. And finally you have a three cross three max pooling okay. So like 1234. So total four important components with respect to the structure okay. And now this particular module in the inception module will be repeated multiple times in the network okay. Now when you are working with a three cross three convolution it will try to capture a medium scale spatial features okay. Whereas a five cross five can actually capture a large scale spatial feature okay. It's very very important. The idea is to look into multiple type of features and not only a same size of feature. So because of that two different type of, I will say convolution operations which has been used.

Now coming to the next part, which is basically the auxiliary classifier. The first thing with respect to because we have how many total auxiliary classifiers, two ox classifiers that we do use. Okay. And what is the use of them. So first it improves the gradient flow. Okay. So when I'm talking about the gradient flow it means that it handles the vanishing gradient problem. As simple as that okay. Then coming to the next step it also acts as a regularization. Okay. So these two are the most important things okay. And finally okay it is used only during training. Okay. So only. In training it is used. So during inferencing you do not require it. Okay. So it is used only for training. So that's why very specifically I will mention no inference. So if you want to do the predictions you don't need to use the auxiliary classifier okay. So it is used only for the purpose of improving the gradient flow okay. And acting as an regularization mechanism okay.

Now coming to the different variants of GoogLeNet. So the first architecture that came around it was in 2014, the inception v1. So 2014 inception. V1. Okay. So then in 2015 starting came the inception v3. Okay. And then again in the uh, last part of 2015 came the inception. Okay. Sorry. So here it will be V2. Okay. And here inception v3. So majorly we use v3. Right now we don't use v1 and V2 because there has been a lot of improvements with respect to the V3 implementation, but very similar architecture that you can look into. Now coming to the next part, what were the major changes? Because if I'm talking with respect to the key improvements. Okay, so here we were using one cross one okay. Uh three cross three five cross five okay. So these were the normal ones. And you were using pooling okay. But again in the V2 paper okay. So batch normalization was introduced okay. So batch normalization was introduced okay. Which helped the network to train faster okay. And the gradients were much more stable. And what they did they broke the five cross five convolution to two times three cross three. So a simple factorization operation which has been done okay.

Now coming to the next part with respect to v3 and v3, what were the major changes. The first one was like uh factorized convolutions. Okay. A very similar implementation just like two times using. So here like if you go with a seven cross seven okay. Now seven cross seven will be broken into. Like it can be three times using three cross three convolution filter okay. Or even like you can break seven cross seven as one, cross seven and seven cross one. So this is also another way okay that they have taken. And based on that uh, the results were much better okay. And very specifically uh, like in previously in V1 and V2 they were using SGD optimizer. But here they have kept it as rmsprop in v3. Okay. So Rmsprop. So these are few of the important things. Okay. And uh, uh, the next part is with respect to image resolution also. So that is also very, very important because if you look here okay. So here the actual resolution okay. So it was 2 to 4 cross 224 okay. So from there so you can see the output size okay. But here they have increased the resolution in V3 the input resolution become 299 cross 299 okay. Previously it was 2 to 4 cross 2 to 4. But this time they have increased the resolution okay. But the architecture is very similar with few improvements with respect to time. And in our practical implementations we will be implementing inception v3. I hope guys you got a basic understanding and the architecture, how it works. The flow. Okay. Please go through the research paper if you have time. All right guys. So see you in the next lecture.

**N) Inception Pretrained Keras**

Hello everyone, welcome to this video on the Inception pre-trained model. In this session, we will be working with Keras instead of PyTorch. The very first step is the basic imports. The idea is to import the InceptionV3 model from keras.applications, along with the preprocessing function required for this model, and finally the decode_predictions utility. These are pre-built functions provided directly within the Keras applications module, so we can conveniently import and use them.

Next, we import a few helper functions such as load_img and img_to_array for image preprocessing, and of course NumPy for array manipulations. After that, we load the pre-trained InceptionV3 model. This can be done with a single call, specifying that we want to use the weights trained on ImageNet. Once the model is loaded, it will be downloaded locally if not already present. We can then check the model summary. As you will see, the input shape starts with a resolution of 299 × 299, which is required by InceptionV3. Since this is a deep and complex architecture, the model summary will be quite long and detailed if you scroll through it.

Now, after loading the pre-trained model, the next step is inference. For this, I created a function predict_image(model, image_path), which handles the full pipeline for prediction. Inside the function, the image is first loaded from the path and resized to 299 × 299. Next, the image is converted into an array and reshaped by adding an extra dimension to simulate a batch (as expected by the model). Then, we apply the preprocessing step using the preprocess_input function from Keras applications. This prepares the image tensor so it matches the distribution expected by the InceptionV3 network.

After preprocessing, the image is passed to the model’s predict method, which generates the predictions. These predictions are then decoded using the decode_predictions function to obtain human-readable class labels along with their corresponding probabilities. Finally, we print the top five predictions for analysis.

To demonstrate this, I tested the model with a simple jellyfish image. The image was loaded from a local path and passed through the prediction pipeline. After resizing to 299 × 299 and running inference, the model predicted “jellyfish” with a probability of 0.95, which is the highest among the predicted classes. Alongside jellyfish, the model also gave related predictions such as “sea animal,” “parachute,” and “swimwear.” However, since jellyfish has the highest probability, the result is accurate and shows that the pre-trained model is performing very well.

So this was a demonstration of using the InceptionV3 pre-trained model for image classification with Keras. In the next lecture, we will move forward and explore transfer learning with InceptionV3, where we will fine-tune the model for custom datasets.

**O) Inception Pretrained Pytorch**

Hello everyone, welcome to this video on implementing the Inception pre-trained model in PyTorch. In this session, we will be downloading the model directly from torchvision and performing inference with it. The first part, as always, involves the basic imports, which we have already discussed multiple times in earlier videos.

Next, we move on to loading the pre-trained model. Here, I am specifically using InceptionV3 instead of v1 or v2, since v3 has several improvements and performs better. We can load it easily using torchvision.models.inception_v3(pretrained=True). Once the model is loaded, we switch it into evaluation mode by calling model.eval(). This is important because we are only doing inference, not training, and evaluation mode ensures layers like dropout or batch normalization behave correctly. After loading, you can also explore the complete architecture of InceptionV3 if you print out the model structure.

The next step involves defining the image transformations. Any image that we pass for inference must be preprocessed correctly to match the model’s expected input. For InceptionV3, the input resolution is 299 × 299, so we resize the image accordingly. After resizing, we convert the image into a tensor and then normalize it using the mean and standard deviation values recommended for ImageNet models. This normalization step ensures that the pixel values are on the same scale as what the model was trained on. Finally, when we open the image, we also make sure to convert it into RGB format before applying these transformations.

With the preprocessing ready, we define the predict function. This function takes in the transformed image tensor and the pre-trained model, runs inference, and returns the predicted output. During inference, we ensure that no gradients are calculated by wrapping the forward pass inside a torch.no_grad() block, since we do not need backpropagation here. The raw predictions are then mapped to human-readable labels by loading the ImageNet class file, which contains all the class names. Without this mapping, the output would only be numerical indices like 0, 1, 2, etc., which are not meaningful to us.

Finally, we come to the visualization part. Here, we take the original image path, apply the transformation and prediction functions, and then plot the image alongside the top-5 predicted classes. This makes it easy to see both the input and the model’s output in one place.

For demonstration, I used an image, passed it through this pipeline, and visualized the results. The model correctly identified the image as a Green Mamba with a confidence of 87.83%. This shows that the pre-trained model is performing well. Since these models are trained on ImageNet, they already recognize a wide variety of classes, so as long as the target class exists in ImageNet, inference becomes straightforward.

So, to summarize, using pre-trained models in PyTorch is quite simple. We just need to load the model, apply the correct transformations, run inference, and then map the predictions back to class labels. This gives us quick and accurate results without any training. That’s all for this session, and in the next lecture, we will continue exploring further concepts related to transfer learning with Inception.

**P) Inception Transfer Learning**

Hello everyone, welcome to this video on transfer learning using the Inception model. This time we will be working with Keras and applying it on the CIFAR-10 dataset. Let me start with a quick introduction to the dataset. The CIFAR-10 and CIFAR-100 datasets are labeled subsets of the larger 80 million tiny images dataset. CIFAR-10 contains ten different classes with a total of 60,000 images, each of resolution 32 × 32 in color (so three channels). Each class has 6,000 images, making up 50,000 training images and 10,000 test images. The classes in CIFAR-10 are: airplane, automobile, bird, cat, deer, dog, frog, horse, ship, and truck. In addition, there is the CIFAR-100 dataset, which has many more classes organized into superclasses, but in this session we will stick to CIFAR-10. Later on, we will also explore CIFAR-100.

Now, let’s move on to the notebook. First, we begin with the imports. From TensorFlow and Keras applications, we import the InceptionV3 model, Keras models, layers, optimizers, and the to_categorical function since we need to convert class labels for training. We also import CIFAR-10 from Keras datasets, along with standard libraries like NumPy and Matplotlib for data handling and visualization. These will be our base imports.

Next, we load the InceptionV3 pre-trained model. I store it in a variable called conv_base. Here, I specify weights="imagenet" so that it uses the ImageNet pre-trained weights. I also set include_top=False, which means we are excluding the final classification layer because our dataset has a different number of classes. Additionally, I define the input_shape as (224, 224, 3). The default input size for InceptionV3 is (299, 299, 3), but since CIFAR-10 images are only 32 × 32, we will later upsample them to match the required resolution. Once loaded, you can check the input shape, which now becomes (224, 224, 3).

At this stage, I also explore the model layers by printing them out. You can see batch normalization layers, activation layers, pooling layers, and many others. The key step now is freezing layers. Freezing means that we do not train certain layers and only keep their pre-trained weights. Since we want to leverage the ImageNet-trained features, I set all the layers as non-trainable by default using layer.trainable = False. This way, training will be much faster and only the top layers we add later will be trained. If we wanted, we could selectively unfreeze some deeper layers for fine-tuning. For example, by setting only the last two layers as trainable, we could customize the training further. But here, I am keeping everything frozen to simplify the process.

Now, let’s look at the dataset loading and preprocessing. CIFAR-10 is loaded using keras.datasets.cifar10.load_data(). After loading, I normalize the images and convert the class labels to categorical using to_categorical. The training set has 50,000 samples and the test set has 10,000 samples.

Next, we define the model architecture. I use a Sequential model where I first perform upsampling because CIFAR-10 images are only 32 × 32. Through repeated upsampling operations, I increase the resolution step by step: 32 × 32 becomes 64 × 64, then 128 × 128, and finally 256 × 256. After that, I apply a resize operation to 224 × 224 with bilinear interpolation, so the input matches what InceptionV3 expects. Then I add the InceptionV3 base model on top of these preprocessing layers. After that, I flatten the output, add a dense layer, and finally a softmax output layer with 10 units corresponding to the CIFAR-10 classes.

With the architecture ready, I compile the model using categorical crossentropy as the loss function, accuracy as the metric, and an optimizer such as Adam. For training, I use a batch size of 64 and train for 10 epochs. Training takes some time, but we can monitor the loss and accuracy. As observed, the validation loss decreased from 1.6 down to 0.85 after 10 epochs, showing that the model is learning well. With more epochs, say 20, 30, or 40, the performance would further improve.

Once training is complete, I save the model in .h5 format using model.save("cifar_model.h5"). This allows us to reload the trained model later without retraining.

Now let’s test the model with a sample prediction. Using OpenCV and Matplotlib, I load an input image (for example, an airplane image), resize it to 224 × 224, and display it. Since we upscaled from 32 × 32, the image looks a bit blurry, but it is still recognizable. Next, I preprocess the image: resize with cubic interpolation, convert to array, expand dimensions, and apply preprocess_input. The processed image is then passed to the trained model for prediction.

The model outputs a probability distribution over the 10 classes. By checking which index has the highest probability, we can identify the predicted class. For example, in one test the predicted index was 0, which corresponds to airplane. Thus, the model successfully classified the image.

So, this was a complete demonstration of transfer learning with InceptionV3 on CIFAR-10. We trained the model by freezing the pre-trained layers, added a few custom layers for CIFAR-10, and achieved good results. In the next video, we will continue this journey and explore transfer learning with ResNet.

**Q) ResNet Architecture**

Hello everyone, welcome to the video on ResNet architecture. In this session, we will explore Residual Networks (ResNet), introduced by Kaiming He et al. and widely regarded as one of the most influential deep learning architectures. The primary motivation behind ResNet was to solve the vanishing gradient problem that arises when training very deep networks. Without ResNet, models deeper than a few dozen layers often suffer from degradation, where adding more layers actually worsens accuracy. ResNet made it possible to train networks with hundreds or even thousands of layers while still maintaining stability and accuracy.

To understand this problem, let’s look at a comparison of a 20-layer network vs. a 56-layer network. Intuitively, the deeper network should perform better, but in reality, the 56-layer model suffers from higher training and test errors due to vanishing gradients. ResNet addresses this with its key innovation: the residual block.

A residual block introduces the idea of a skip (shortcut) connection, where instead of only relying on stacked layers to learn mappings, the input x is directly added back to the transformed output F(x). Mathematically, the block learns y = F(x) + x. This ensures that even if intermediate convolutions lose some information, the original input is preserved. This concept provides three main benefits: feature reuse, solving vanishing gradients, and preventing degradation when layers increase.

There are two types of residual blocks. The first is the basic block, which uses two 3×3 convolutions. The second is the bottleneck block, which is more efficient for deeper networks. It applies a 1×1 convolution → 3×3 convolution → 1×1 convolution structure. The 1×1 convolutions reduce and then restore dimensionality, making computation much more efficient. Typically, ResNet-18 and ResNet-34 use basic blocks, while ResNet-50, ResNet-101, and ResNet-152 use bottleneck blocks.

Inside each residual block, the typical structure includes 2–3 convolution layers, batch normalization, ReLU activations, and skip connections. The skip connections allow gradients to flow directly through the network, bypassing intermediate transformations, which is why ResNet avoids vanishing gradients. Additionally, identity mappings ensure that features learned in earlier layers are preserved and reused in later layers.

Now, let’s look at the overall ResNet architecture. The general flow is:

(i) Initial convolution + max pooling (7×7 conv with stride 2, followed by pooling).

(ii) Multiple residual blocks (basic or bottleneck depending on the variant).

(iii) Global Average Pooling (GAP) to reduce spatial dimensions.

(iv) Fully connected (FC) layer + softmax for classification (1000 classes in ImageNet).

For example, in ResNet-18, each block has two 3×3 convolutions and is repeated in stages. ResNet-34 increases the repetitions (e.g., 3, 4, 6, 3). In deeper versions like ResNet-50, 101, 152, bottleneck blocks are used, where the 1×1 convolutions make training feasible even with hundreds of layers. For instance, ResNet-50 has 3 bottleneck blocks in stage 1, 4 in stage 2, 6 in stage 3, and 3 in stage 4, followed by GAP and a fully connected layer.

To summarize, ResNet’s success comes from two simple yet powerful ideas: skip connections (which allow gradients to bypass multiple layers) and identity mapping (which ensures earlier features are preserved). These innovations made very deep neural networks practical and are still widely used in state-of-the-art models today.

In upcoming lectures, we will implement ResNet using PyTorch and Keras, and see firsthand how it works in practice.

**R) Resnet Pretrained Keras**

Hello everyone, welcome to this lecture on the ResNet50 pre-trained model using Keras. Just like in our previous sessions, here also we will be using the pre-trained model available in Keras applications. To begin with, we need to import the required modules. From keras.applications, we import the ResNet50 model along with preprocess_input and decode_predictions functions, which help in preparing the image and decoding the output respectively. Additionally, we import helper functions like load_img and img_to_array from keras.preprocessing.image for image handling. For array manipulations, we import NumPy, and for visualization, we import Matplotlib. The imports can be written as "from keras.applications import ResNet50, preprocess_input, decode_predictions", "from keras.preprocessing.image import load_img, img_to_array", "import numpy as np", and "import matplotlib.pyplot as plt".

Once the imports are ready, we move on to downloading and loading the model. This is done with "model = ResNet50(weights='imagenet')". The first time you run this, the weights will be downloaded locally. After loading, we can check the architecture by using "model.summary()". This will print a very detailed and long summary since ResNet50 is a deep architecture. If you scroll through the summary, you will notice that the model has around 25 million parameters, which makes it very powerful for large-scale image classification tasks.

Now that the model is loaded, let us move towards prediction. For that, I created a helper function called "def predict_image(model, image_path):". Inside this function, we start by loading the image with "img = load_img(image_path, target_size=(224, 224))". ResNet50 expects input images of size 224 × 224, so resizing is necessary. Then, we convert the image into an array using "img_array = img_to_array(img)". Since the model expects batches, we add an extra dimension with "img_batch = np.expand_dims(img_array, axis=0)". After this, preprocessing is applied using "processed_img = preprocess_input(img_batch)". Finally, we run the model’s prediction with "predictions = model.predict(processed_img)" and decode the results with "decoded = decode_predictions(predictions, top=5)[0]".

For demonstration, I used a simple zebra image. The image was first loaded and displayed using "plt.imshow(img)" and "plt.axis('off')". Then I passed its path to the predict_image function. When predictions were printed, the output displayed the top five classes along with probabilities. For example, "('n02391049', 'zebra', 0.99)" was the top prediction, followed by other animals like gazelle, cheetah, and tiger, but with much lower probabilities. This confirms that the model correctly identified the zebra with very high confidence of 99%.

Thus, we can clearly see that the ResNet50 pre-trained model works very well out of the box because it has already been trained on the large ImageNet dataset. This makes it highly reliable for recognizing common objects. So this was a simple end-to-end demonstration of using ResNet50 with Keras. In the upcoming lecture, we will also explore how to implement ResNet50 using PyTorch, and later, we will extend this into transfer learning for custom datasets.

**S) Resnet Pretrained Pytorch**

Hello everyone, welcome to this video on the ResNet pre-trained model using PyTorch. In the previous lecture, we implemented this in Keras, and now we will quickly go through the same process using PyTorch.

First, we begin with the basic imports. We import PyTorch and TorchVision since the ResNet models are available inside TorchVision. For example, "import torch", "import torchvision.models as models", and "import torchvision.transforms as transforms". Additionally, to read the input image we make use of the Python Imaging Library, so we import "from PIL import Image".

The next step is to load the pre-trained model. This can be done with "model = models.resnet50(weights='IMAGENET1K_V1')". Once the model is downloaded, we switch it into evaluation mode using "model.eval()". This step is important because it ensures that the network behaves in inference mode (i.e., dropout and batch normalization layers are handled properly). As expected, ResNet50 is a large architecture that makes use of bottleneck layers, and you can see this in the printed summary if you explore the model.

After loading the model, the next focus is image preprocessing. The image has to be resized, converted to a tensor, and normalized using the same mean and standard deviation values as ImageNet. For this, we define a transform pipeline using "transform = transforms.Compose([transforms.Resize(256), transforms.CenterCrop(224), transforms.ToTensor(), transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])". This ensures that the input matches what ResNet expects.

Now let us take the same zebra image we used earlier. We load the image with "img = Image.open('zebra.jpg')". Once the image is opened, we apply the transformation using "img_t = transform(img)". Since the model expects a batch, we add an extra dimension with "batch_t = torch.unsqueeze(img_t, 0)".

With preprocessing complete, we can move on to inference. We ensure no gradients are tracked by using "with torch.no_grad():". Inside this block, we compute the prediction as "out = model(batch_t)". To extract the top three predictions, we use "_, indices = torch.topk(out, k=3)".

In order to interpret these predictions, we also need the class labels. In Keras we loaded them from an external file, but in PyTorch the ResNet50 weights already include metadata with category labels. So we can load them using "labels = models.ResNet50_Weights.IMAGENET1K_V1.meta['categories']". This gives us direct access to the ImageNet class names.

Finally, we print the predictions. For example, "for idx in indices[0]: print(labels[idx])". In this case, the top prediction is zebra with the highest probability, followed by other related classes. This shows that the model has correctly recognized the zebra. To visualize the input, we can also display it with "img.show()".

So, in summary, we have successfully implemented the ResNet50 pre-trained model using PyTorch. It was a very quick and straightforward process, involving model loading, preprocessing, inference, and decoding predictions. In the upcoming lectures, we will extend this further into transfer learning using ResNet50.

**T) Resnet Transfer Learning**

Hello everyone, welcome to this lecture on Transfer Learning using ResNet50 in PyTorch. In the last session, we worked with pre-trained models, and this time we will extend it to transfer learning. Let’s get started.

The first step is importing the required libraries. We import PyTorch with "import torch" and its neural network module "import torch.nn as nn". For optimization we use "import torch.optim as optim". From TorchVision, we import both models and datasets: "import torchvision.models as models", "import torchvision.datasets as datasets", and "import torchvision.transforms as transforms". Additionally, we bring in "import matplotlib.pyplot as plt", "import numpy as np", and "import os" for visualization, numerical operations, and file handling.

Next, we check whether GPU is available. This can be done with "device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')". In my case, yes, a GPU is available, and specifically I’m using an L4 GPU, which is more powerful than the usual T4. Since our dataset is larger this time, this GPU will speed up training.

Now we define our default image transformations. For every image, we resize, convert it to a tensor, and normalize using ImageNet statistics. The code is "transform = transforms.Compose([transforms.Resize((224,224)), transforms.ToTensor(), transforms.Normalize([0.485,0.456,0.406], [0.229,0.224,0.225])])".

The dataset we are using here is the fruits dataset, which contains five classes: apple, banana, grape, mango, and strawberry. The folder structure contains separate directories for train, test, and validation. For example, inside train/strawberry/ you’ll find all strawberry images. I mounted my Google Drive, so the dataset path points to "drive/MyDrive/fruits". For loading, we use "train_data = datasets.ImageFolder(train_dir, transform=transform)" and "test_data = datasets.ImageFolder(test_dir, transform=transform)". The DataLoader is created as "train_loader = torch.utils.data.DataLoader(train_data, batch_size=30, shuffle=True)" and "test_loader = torch.utils.data.DataLoader(test_data, batch_size=30, shuffle=False)".

Checking dataset info, we have around 9,700 images in total across the five classes, with class distribution like ~1,940 per fruit. The validation split is much smaller with about 2,020 images. So, compared to earlier examples, this dataset is bigger and more challenging.

Now let’s load the model. We start with "model = models.resnet50(weights=models.ResNet50_Weights.IMAGENET1K_V1)". Since ImageNet has 1000 classes and our dataset has only 5, we need to modify the final classification layer. First, we extract the input features with "num_ftrs = model.fc.in_features", then replace the fully connected layer with "model.fc = nn.Linear(num_ftrs, 5)". Finally, we move the model to GPU with "model.to(device)".

We define the loss function as "criterion = nn.CrossEntropyLoss()" and the optimizer as "optimizer = optim.Adam(model.parameters(), lr=0.001)".

Now comes the training loop. We set epochs to 5: "num_epochs = 5". For each epoch, we set the model to training mode with "model.train()", zero out the gradients using "optimizer.zero_grad()", and then forward pass as "outputs = model(images)". Loss is calculated with "loss = criterion(outputs, labels)", followed by backpropagation "loss.backward()" and optimizer step "optimizer.step()". We also track the running loss per epoch. To measure the time, we can use "import time" and "start = time.time()".

For evaluation, we switch to evaluation mode with "model.eval()", disable gradients "with torch.no_grad():", and then pass the test data to get predictions. Accuracy is computed by comparing predicted labels with ground truth.

Next is inference on a single image. Suppose we load "img = Image.open('test/strawberry/001.jpg')". We apply the transform, add a batch dimension "img = img.unsqueeze(0)", and pass it through "outputs = model(img.to(device))". The predicted class index is obtained with "_, preds = torch.max(outputs, 1)". Mapping this index back to class names, we can print the fruit prediction.

Finally, visualization. We can fetch a batch from the validation loader, move them to GPU, run inference, and then display the images with predictions using Matplotlib. For example: "plt.imshow(np.transpose(img.cpu().numpy(), (1,2,0)))" along with the predicted label in the title.
...
Hey everyone. Welcome back. So far you have been exploring computer vision concepts like image classification and object detection. In this module, we are going to focus on segmentation. We will learn concepts related to segmentation like loss function architecture changes and will also perform tasks like custom architecture build as well as model training using the latest computer vision framework. In this lecture, we are going to understand what image segmentation is. Let's get started.

We will start by understanding the definition of image segmentation. Image segmentation is a computer vision technique that partitions an image into multiple segments or regions to simplify its representation and make analysis more meaningful. To understand image segmentation better, we will first recall what we were doing in image classification as well as object detection.

If I talk about image classification, given the image to our CNN model—suppose this is our CNN model and this is our image—we get a prediction of the label. The label can be anything based on the training dataset we have. It can be dog if the picture is of a dog, it can be cat, it can be a person, or it can be X label. X is a variable here and can change. So given the image to the CNN model, it predicts a label for the whole image. This is what we do in image classification.

Now, if I talk about object detection, this is how the flow looks like. We have an image and we have our object detector model. It predicts multiple things. First of all, it will predict xy coordinates, confidence, and also the class label (CLS label). Given the image, it predicts what are the multiple objects or the different objects present in my image and labels for those particular boxes. This particular prediction is for a single entity. So in classification, for the whole image we get a single output. In terms of object detection, given the image, if there are multiple objects present, for each object, we get this entity. So in my image, if I have two dogs, I will get two of these boxes for dogs and three of these boxes for person. This is what happens in terms of object detection.

Now let's talk about image segmentation. Unlike image classification, which labels the entire image, or object detection, which draws a bounding box around objects, image segmentation assigns a label to each pixel, which effectively creates a mask that outlines the exact shape of those objects which we were detecting in object detection. Suppose again this is an image made up of lots of pixels. In segmentation or image segmentation, what we do for each pixel is classification. This pixel belongs to a dog, cat, or person, assuming we are training a model on these three classes. Each pixel will be classified to either dog, cat, or person. Here, we do pixel-level classification.

Segmentation provides pixel-level classification, knowing exactly which pixels belong to an object rather than just drawing a box around it. Suppose we have a dog in three pixels. Instead of drawing a bounding box, which object detection would do, segmentation will identify exactly which pixels are part of the dog. This is crucial for tasks like autonomous driving, medical imaging, and scene understanding. In object detection, even though a pixel does not belong to a dog, it could still be classified as part of the dog due to the bounding box. In image segmentation, we know which pixel belongs to which object, truly seeing the world.

For example, in autonomous driving, suppose we have a car and a person. In object detection, if I draw a box around the car and person, the box could also cover the road, trees, or occluded objects. We are not able to see pixel-level understanding here. The problem segmentation solves includes accurate object boundaries, understanding object shapes and contours, foreground versus background separation, and more detailed feature detection for downstream tasks. Real-world use cases include medical imaging, self-driving cars, agriculture (detecting disease in crops), satellite imaging (land cover classification like forest, water bodies, and buildings), and augmented reality (accurately placing virtual objects in a physical scene).

We have performed a simple level of segmentation in OpenCV classes using grayscale images. Suppose we have a grayscale image and write a condition: if a pixel is above a certain threshold, it should be one; else, it should be zero. After applying this condition, we get an image with only two different values, effectively creating a mask. This mask has been used in OpenCV lectures to remove backgrounds, extract foregrounds, and perform other operations. However, this simple thresholding does not work for complex objects like a person because a person does not have a single threshold. Instead, we need computer vision algorithms where each pixel can be classified as person, dog, cat, car, or any object based on the dataset. Segmentation is not one-size-fits-all; depending on the task, a different kind of segmentation technique might be needed.

There are three main deep learning-based segmentation techniques: semantic segmentation, instance segmentation, and panoptic segmentation. Semantic segmentation involves classifying each pixel in an image into a category. Instance segmentation is an advanced version of semantic segmentation that identifies individual instances of an object. Panoptic segmentation is a combination of semantic and instance segmentation.

In semantic segmentation, each pixel is given a class. Suppose we are looking for a person, the class we are interested in. All persons will be mapped with one color (white), and all unwanted pixels will be zero (black). For multiple classes, each object gets a unique label. For example, cars could be labeled as two, persons as one, and everything else as zero (background). Given an RGB image to a CNN model, we get a mask as output. This can be a binary mask or multi-class mask and can have the same size as the original image depending on the CNN architecture.

Instance segmentation does everything semantic segmentation does, plus it identifies individual instances of an object. Given an image, instance segmentation works like an object detection algorithm. We first get a bounding box and class detection, then pass that cropped region for mask generation. The output includes bounding boxes, class labels, and masks. This allows us to count instances, which is not possible with semantic segmentation alone. Instance segmentation is superior because it combines object detection with mask generation, distinguishing multiple instances of the same class. Background detection is not performed in instance segmentation because pixel-wise classification is done only within detected objects.

Panoptic segmentation is a combination of instance segmentation and semantic segmentation. Here, we perform pixel-level classification for the whole image plus distinguish individual instances. While panoptic segmentation is conceptually powerful, it is rarely used in the industry due to complexity and limited use cases. In real-world applications, instance segmentation and semantic segmentation are most common. Semantic segmentation is used in medical imaging and land cover classification. Instance segmentation is used in e-commerce for product segmentation, among other use cases. Instance segmentation outputs can be post-processed to produce semantic segmentation outputs, but semantic segmentation outputs cannot be converted to instance segmentation because instance-level information is missing. Panoptic segmentation can be derived from instance segmentation outputs if required.

Finally, you have learned the three main types of image segmentation, how they differ, the problems they solve, and how they work internally. That’s all for this lecture. I will see you in the next one.

**B) Downsampling**

Hello everyone, and welcome back to another exciting lecture. In this lecture, we are going to explore a fundamental concept in image processing and deep learning called downsampling. This technique is crucial for handling large images, speeding up computation, and even preventing overfitting in convolutional neural networks. Let's break it all down and understand what it does.

We will start by defining downsampling. Downsampling is the process of reducing the spatial resolution of an image or feature map. It decreases the number of pixels while retaining the most essential information. If you recall this property, you might guess that downsampling is basically your max pooling or average pooling layers, but with respect to segmentation, it is called downsampling. Think of this lecture as a recap of your previous lectures, focusing on the pooling part. Just like downsampling, we have a term coming in future lectures called upsampling, which will be used in instance segmentation or semantic segmentation algorithms.

The common downsampling techniques include averaging (average pooling, taking the average value of neighboring pixels), max pooling (taking the maximum value in a local window), and global average pooling (which performs pooling on the whole image and gives a single value per channel). The output shape is determined by a formula based on the kernel size and stride. Downsampling reduces the spatial resolution of the image, not the number of channels. Resolution refers to the width and height of the image (e.g., 30×30), while channels refer to features (e.g., 128, 256). We reduce the number of channels using 1×1 convolutions and reduce spatial resolution using pooling operations like average pooling, max pooling, or global average pooling.

To understand why downsampling is used, consider an image of size 6464×3 (RGB). The total number of pixels is approximately 1.2 million. If we were to feed this image directly into a fully connected neural network, we would need 1.2 million neurons in the first hidden layer just to represent all pixels. To process this image effectively, the network would need a dense structure with many parameters, which is computationally expensive and prone to overfitting. Additionally, a high-resolution image contains a lot of background and noise, which is redundant and unnecessary for the model to learn. Downsampling or pooling helps by reducing spatial resolution, keeping only the most prominent features, and discarding noise.

Downsampling makes operations more efficient, enabling faster training and inference because we now deal with compressed information instead of millions of pixels. It also prevents overfitting by reducing the number of parameters and limits memory usage, making it easier to handle large images on GPUs. For example, if we have a 4×4 matrix like [[1,3,2,4],[5,6,7,8],[9,2,3,1],[4,0,6,2]] and apply a max pooling operation in PyTorch with a kernel size of 2×2 and stride 2, the output will be a 2×2 matrix [[6,8],[9,6]], retaining the most prominent values from the original feature map.

We can replicate this in Python using PyTorch as follows: we first import libraries with import torch and import torch.nn.functional as F. We create the tensor using image = torch.tensor([[1,3,2,4],[5,6,7,8],[9,2,3,1],[4,0,6,2]], dtype=torch.float). We add dimensions for batch and channel using image = image.unsqueeze(0).unsqueeze(0). Checking the dimensions with image.shape shows torch.Size([1,1,4,4]). We can apply max pooling with max_pooled = F.max_pool2d(image, kernel_size=2, stride=2) and print the output using print("Max pooled output is", max_pooled.squeeze()). Similarly, a strided convolution can perform downsampling: conv_layer = torch.nn.Conv2d(in_channels=1, out_channels=1, kernel_size=3, stride=2) and conv_output = conv_layer(image) gives the strided convolution output, which also reduces spatial resolution.

Both max pooling and strided convolution reduce the input dimensions (from 4×4 to 2×2 in this example), but they work differently. Max pooling simply takes the maximum value in a window, while convolution is learnable and can extract features based on the kernel weights. Adjusting the kernel size and stride changes the output resolution. For instance, a larger kernel or stride produces lower resolution outputs. The key takeaway is that both max pooling (F.max_pool2d) and strided convolution reduce the spatial resolution of input features or images, making operations efficient and helping prevent overfitting.

By understanding these operations and running the provided code, you can see how downsampling works and why it is necessary in convolutional neural networks, especially when handling high-resolution images. Thanks for watching this lecture. I will see you in the next one.

**C) Upsampling/Transposed Convolution**

Hey everyone, welcome back. In the previous lecture, we explored downsampling, where we reduced image size to make models more efficient. Today, let's flip that concept and dive into upsampling and transposed convolutions, techniques that increase image size, which is critical for tasks like image segmentation and image generation.

Before getting to transposed convolution, we first need to understand what upsampling is. Upsampling is the process of increasing the spatial resolution of an image or feature map. In downsampling, we compress and reduce spatial resolution, whereas upsampling increases it. Conceptually, it is like stretching a small image to make it larger while preserving the original structure. Common upsampling methods include nearest neighbor interpolation, where we copy the nearest pixel value, and bilinear interpolation, which uses a weighted average of nearby pixels to fill in new pixels. For example, consider a 2×2 image with pixel values "image = torch.tensor([[1,2],[3,4]], dtype=torch.float)". Upsampling this image to 4×4 will involve filling in additional pixels based on the chosen algorithm. Using nearest neighbor interpolation in PyTorch, we can write "UpsampledKNN = F.interpolate(image.unsqueeze(0).unsqueeze(0), scale_factor=2, mode='nearest')", and this expands the 2×2 image to 4×4 by repeating pixel values.

However, these interpolation methods are not part of a CNN’s learnable architecture because they are fixed algorithms. This is where transposed convolution (also called deconvolution) comes into play. Transposed convolution learns how to upsample feature maps or images using trainable filters. Unlike interpolation, transposed convolution has learnable parameters that can be optimized through backpropagation using gradient descent. This makes it possible to include transposed convolution layers directly within a CNN architecture. In PyTorch, a simple transposed convolution layer can be initialized with "conv_transpose = torch.nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=3, stride=2, padding=1)", and applied to our image tensor with "output_transpose = conv_transpose(image.unsqueeze(0).unsqueeze(0))".

Why do we need upsampling? After multiple downsampling operations (via convolution and pooling), feature maps become compressed and low-resolution. For tasks like image segmentation, generative models, or super-resolution, we need to reconstruct high-resolution outputs. For example, a compressed feature map of size 16×16×512 must be upsampled to 512×512×3 to restore spatial details. Upsampling ensures that pixel-level outputs are accurately reconstructed, which is essential for semantic segmentation, medical imaging, image denoising, and style transfer. Style transfer uses upsampling to enlarge stylized outputs while preserving artistic features, applying a desired style to an original image.

Let’s see the upsampling in action in PyTorch. First, we create a small 2×2 tensor as "image = torch.tensor([[1,2],[3,4]], dtype=torch.float)". We then add batch and channel dimensions using "image = image.unsqueeze(0).unsqueeze(0)". Using nearest neighbor interpolation, we can upsample it to 4×4 with "UpsampledKNN = F.interpolate(image, scale_factor=2, mode='nearest')" and view the output using "print('Nearest neighbor upsampling:', UpsampledKNN.squeeze())".

Next, we can apply transposed convolution, which learns optimal pixel values rather than just repeating them. We initialize the layer as "conv_transpose = torch.nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=3, stride=2, padding=1)" and pass the image tensor through it with "output_transpose = conv_transpose(image)". Viewing the result with "print('Transposed convolution upsampling:', output_transpose.squeeze())" shows how the tensor is expanded, and the values are determined by learnable filters. Adjusting the stride or kernel size changes the output resolution.

The key difference is that interpolation stretches the matrix by copying or averaging values, while transposed convolution expands the matrix by learning new pixel values through convolution, optimizing them to minimize the loss. For example, suppose we have a compressed 4×4 feature map obtained after several convolutions and pooling operations. Using transposed convolution "conv_transpose(feature_map)" allows the network to map these low-resolution features back to the original image size, restoring fine details based on learned parameters. This is essential in tasks like semantic segmentation, image generation, and super-resolution, where high-resolution outputs are required.

By the end of this lecture, you can see that nearest neighbor interpolation and bilinear interpolation are simple algorithms to increase image resolution, whereas transposed convolution is a learnable operation that can be integrated into neural networks to upsample feature maps optimally. This technique allows CNNs to generate high-resolution outputs from compressed, abstract feature representations, solving the challenge of reconstructing spatial details after downsampling.

**D) Segmentation Loss Functions**

Hey everyone, welcome back. Now that we have explored image segmentation and techniques like upsampling and transposed convolution, we need to understand how to measure performance during training of a segmentation algorithm. In this lecture, we will break down segmentation loss functions, the core component that guides our model to improve.

In deep learning, a loss function quantifies the difference between model predictions and the ground truth. In image segmentation, we need loss functions that compare pixel-level classification. For example, in image classification, we have an image and the ground truth (actual label). If the ground truth is “dog” but our model predicts “cat”, we calculate accuracy or loss, and through backpropagation, gradients are computed and updated using gradient descent, allowing the model to learn.

However, in segmentation, we do not generate a label for the overall image. Each pixel belongs to an individual class. For instance, if our image is 640×640 and we have a binary mask (label one), we generate 640×640 predictions—each pixel needs classification. This introduces the need for specialized loss functions because standard losses like mean squared error (MSE) do not handle class imbalance well. Segmented images often have small object regions (e.g., tumors in medical images or houses in satellite images) compared to large background areas. This class imbalance means that the model may focus more on predicting background pixels correctly rather than the small but important object regions.

Segmentation loss functions help the model focus on the actual labels of interest rather than the background. Some commonly used losses include cross-entropy loss, which classifies each pixel but struggles with class imbalance, and IOU loss, which directly optimizes the intersection-over-union metric. Dice loss, however, emphasizes the intersection between prediction and ground truth, making it suitable for segmentation tasks.

The formula for binary cross-entropy loss is "L_CE = -1/n Σ (y_i * log(y_hat_i))", where y_i is the true label (0 or 1) and y_hat_i is the predicted probability. Dice loss is formulated as "L_Dice = 1 - (2 * Σ(y * y_hat) / (Σ y + Σ y_hat))", where the numerator calculates the intersection and the denominator the sum of predicted and ground truth areas. Dice focuses more on intersecting pixels, ensuring that the model learns regions that matter most, rather than being dominated by background pixels.

To illustrate in PyTorch, we can define dummy tensors for predictions and target masks as "prediction = torch.tensor([0.9, 0.1, 0.8], dtype=torch.float)" and "target = torch.tensor([1.0, 0.0, 1.0], dtype=torch.float)". For binary cross-entropy loss, we can write "bce_loss = F.binary_cross_entropy(prediction, target)".

Dice loss can be implemented as a function:
...
Here, the smooth term prevents division by zero, which can happen when the ground truth mask has no positive pixels. We can combine both losses into a segmentation loss function:

"def segmentation_loss(prediction, target, weight=0.005):
 bce = F.binary_cross_entropy(prediction, target)
 dice = dice_loss(prediction, target)
 return weight * bce + dice"

By default, we assign a small weight (0.5%) to BCE and give most emphasis to Dice loss. If we want equal weightage, we can set weight=1.0. The combined segmentation loss ensures that the model learns to predict the correct pixels while still considering class imbalance and intersection regions, optimizing both pixel-level accuracy and spatial overlap.

Using this approach, our segmentation loss focuses on the predictions that are correct (intersecting pixels) and emphasizes the important class labels. This combination of Dice loss and (binary) cross-entropy loss is widely used in segmentation tasks, such as medical imaging, satellite imagery, and other applications requiring precise pixel-level predictions.

That's all for this lecture. Thanks for watching, and I will see you in the next one.

**E) Fully Convolutional Networks (FCNs)**

Hey everyone, welcome back. In this lecture, we will learn about Fully Convolutional Networks, or FCNs for short. FCNs are one of the earliest deep learning architectures specifically designed for image segmentation. We will explore what an FCN is, understand the standard components mentioned in its research paper, and implement a similar architecture using PyTorch.

Before diving into FCNs, we need to understand the limitations of traditional convolutional neural networks (CNNs). Consider a standard CNN that takes an input image and passes it through a series of convolutional layers, ReLU activations, max pooling, and batch normalization—essentially the typical convolutional blocks found in vanilla CNNs. The output of such a network is a dense feature representation that is flattened and connected to fully connected layers. For example, if there are three classes to predict, the final layer will have three neurons, and a softmax activation will provide class probabilities. This works well for image classification, but not for image segmentation. In segmentation, the output is not a single label but another image where each pixel is classified into a category. Therefore, traditional CNNs are not suitable for pixel-wise predictions.

This is where FCNs come in. A Fully Convolutional Network is a deep learning architecture in which fully connected layers are replaced with convolutional layers. Unlike standard CNNs that reduce an image to a single class label, FCNs maintain spatial information throughout the network, making them ideal for pixel-wise segmentation tasks. In FCNs, the convolutional layers extract features as usual, but the final output is a set of feature maps where each channel corresponds to a particular class. For example, for an image with three classes—person, cat, and aeroplane—the final output will have three channels, each providing a segmentation map for one class. The network learns to predict pixel-level class labels through backpropagation, maintaining spatial resolution from input to output.

The original FCN research paper introduced several key components for semantic segmentation. The architecture starts with convolution and pooling layers (like a vanilla CNN), followed by additional convolution layers that replace the fully connected layers. One important technique is the use of skip connections: the network combines outputs from intermediate pooling layers with deeper layers to capture both low-level and high-level features. Different FCN variants exist, such as FCN-32, FCN-16, and FCN-8, which progressively refine the segmentation output by integrating features from multiple layers. For instance, FCN-32 uses only the final convolution output, FCN-16 adds features from the fourth pooling layer, and FCN-8 further integrates features from the third pooling layer. This combination improves the quality of the segmentation mask, although the output may still be less precise than the ground truth.

In PyTorch, we can implement an FCN by using a pre-trained backbone like VGG16. The goal is to extract intermediate feature maps from the third and fourth max pooling layers, as well as the final output. These feature maps are passed through 1×1 convolutions to reduce the number of channels to the number of classes. Since the spatial resolutions of these feature maps differ, we apply transposed convolutions (also called deconvolutions) to upsample the lower-resolution maps to the same size before adding them together. Specifically, we use upsampling factors of 32, 16, and 8 to scale feature maps from different layers so they can be merged, and finally, we upsample the combined feature map to match the original input size. By following this process, the network produces an output of the same spatial dimensions as the input image, with the number of channels corresponding to the number of classes, effectively providing pixel-wise predictions.

Finally, after constructing the network with all components—including VGG16 feature extraction, 1×1 convolutions, skip connections, and upsampling layers—we can test it using a dummy input image. The network output preserves the spatial dimensions of the input image while producing the desired number of class masks. This implementation demonstrates how FCNs solve the problem of pixel-wise segmentation by maintaining spatial information throughout the network and combining multi-scale feature maps to produce accurate segmentation outputs.

**F) UNet**

Hey everyone, welcome back. In the previous lecture, we learned about Fully Convolutional Networks (FCNs), a special type of CNN designed specifically for image segmentation. In this lecture, we will take a step further and understand U-Net, which is another fully convolutional neural network designed primarily for biomedical image segmentation. U-Net is known for its U-shaped architecture. While it was initially introduced for biomedical applications, it works well across other domains and can perform effectively even with small datasets. By the end of this lecture, you will understand what U-Net is and why it outperforms traditional FCNs.

Unlike conventional definitions of neural networks, U-Net is better described through its key characteristics as a segmentation architecture. As the diagram of U-Net shows, the network has a distinctive U-shaped structure composed of two main parts: the contracting path, also called the encoder, and the expanding path, also called the decoder. The encoder compresses the input data, extracting meaningful features, while the decoder expands these features to reconstruct the output with pixel-wise predictions. U-Net also uses skip connections, which allow features from earlier layers in the encoder to be directly passed to the corresponding layers in the decoder. These skip connections help retain fine details that might otherwise be lost during downsampling, enabling the network to segment images accurately even with very few training samples.

You might wonder why U-Net is considered superior to a traditional FCN. While FCNs also employ skip connections—typically from pooling layers three, four, and five—U-Net directly connects each encoder layer to its corresponding decoder layer. This direct connection allows the network to pass fine details at multiple resolutions, which are critical for accurately segmenting small or complex objects. In contrast, FCNs primarily upsample the final feature maps to match the original input resolution without directly utilizing intermediate features from the contracting path.

Another key advantage of U-Net is its symmetric U-shaped design. Unlike FCNs, which mainly perform upsampling of the final feature map, U-Net mirrors the contracting path during decoding. This mirroring allows the decoder not only to upsample features but also to recover information lost during encoding, improving feature retention and localization. Consequently, U-Net preserves fine-grained details and is particularly effective at segmenting small objects, which FCNs often struggle with due to aggressive downsampling.

Additionally, while FCNs typically use only transposed convolution for upsampling, U-Net refines the upsampling features using additional convolution layers. This results in better localization and improved segmentation accuracy. Because of its architecture and skip connections, U-Net can achieve high performance with smaller datasets, even without extensive data augmentation. This is particularly important in biomedical imaging, where data is often limited, and the objects of interest, such as cells or tumors, are small.

The original U-Net research paper, published in 2015 on arXiv, used input images of size 512×512 and detailed the full architecture, including convolution operations, max-pooling layers, upsampling layers, and skip connections. The architecture follows a clear repetitive pattern: two sets of convolutions at each stage of the encoder, doubling the number of channels as we go deeper, followed by the decoder, where the feature maps are upsampled while concatenating corresponding encoder features. Finally, a 1×1 convolution reduces the number of channels to match the number of target classes. This structured design makes U-Net simple to implement while ensuring high segmentation accuracy.

In conclusion, U-Net is more than just an improvement over FCNs. It is a go-to architecture for segmentation tasks that require high spatial accuracy and effective detection of small objects. Its use of skip connections, symmetric U-shaped design, and refined upsampling make it highly suitable for biomedical image segmentation and other tasks where preserving fine details is crucial. In the next lecture, we will see how to implement U-Net using PyTorch in Google Colab.

Thanks for watching, and I’ll see you in the next lecture.

**G) Implementing Custom Unet Training**

Hey everyone, welcome back! In the previous lecture, we learned about U-Net and why it is superior to traditional Fully Convolutional Networks (FCNs). In this lecture, we will implement a custom U-Net along with all the components required for training it. Let’s get started.

The dataset we are going to use for training the U-Net model is taken from a GitHub repository authored by Vikram Shenoy. Most images were downloaded from Google Image Search, while some were taken from other datasets. Since the original dataset did not include masks, they were generated using an API to remove the background. This is a semantic segmentation task where the only class is "person": all segmented parts of the person are white, and the background is black. The ground truth images are stored in PNG format, which allows storing alpha channels if needed. Note that the dataset may contain some noise, so results may not be completely accurate. The goal is to demonstrate how to create a U-Net, train it, reduce loss, and obtain a model for inference.

The dataset is structured such that for each training image (e.g., 1.jpg) there is a corresponding mask in the ground_truth folder (e.g., 1.png). To avoid downloading a zip, we can clone the repository directly into Google Colab. We will also enable a GPU runtime, selecting a Tesla T4 with 15GB memory to train our model efficiently. After cloning, the folder structure will contain training_images and ground_truth.

We start by importing the necessary libraries: "import os, time, torch, torch.nn as nn; from PIL import Image; from torchvision import transforms; from torch.utils.data import Dataset, DataLoader". We will use transforms to resize and convert images to tensors, and DataLoader to retrieve data in batches.

Next, we define the custom segmentation dataset class. This class takes image_dir and mask_dir as input, filters valid image extensions, applies resizing to (512, 512) and converts the images to tensors. We also threshold the mask to ensure it is binary. The class looks like: "python class SegmentationDataset(Dataset): def __init__(self, image_dir, mask_dir): self.image_dir = image_dir self.mask_dir = mask_dir self.transform = transforms.Compose([ transforms.Resize((512, 512)), transforms.ToTensor() ]) valid_extensions = ['.jpg', '.jpeg', '.png'] self.images = [f for f in os.listdir(image_dir) if os.path.splitext(f)[-1].lower() in valid_extensions] def __len__(self): return len(self.images) def __getitem__(self, idx): image_path = os.path.join(self.image_dir, self.images[idx]) name, ext = os.path.splitext(self.images[idx]) mask_path = os.path.join(self.mask_dir, f"{name}.png") image = Image.open(image_path).convert('RGB') mask = Image.open(mask_path).convert('L') image = self.transform(image) mask = self.transform(mask) mask = (mask > 0.5).float() return image, mask ".

Once the dataset is ready, we define a helper function to create a DataLoader. This allows batching and shuffling, which improves training: "python def get_data_loader(image_dir, mask_dir, batch_size=2, shuffle=True): dataset = SegmentationDataset(image_dir, mask_dir) return DataLoader(dataset, batch_size=batch_size, shuffle=shuffle) ".

Next, we build the U-Net architecture. The first block is the double convolution block. It performs two convolutions with kernel size 3 and padding 1, each followed by a ReLU activation: "python class DoubleConv(nn.Module): def __init__(self, in_channels, out_channels): super().__init__() self.conv = nn.Sequential( nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1), nn.ReLU(inplace=True), nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1), nn.ReLU(inplace=True) ) def forward(self, x): return self.conv(x) ".

For downsampling in the encoder, we create a class that uses max pooling followed by double convolution: "python class DownSample(nn.Module): def __init__(self, in_channels, out_channels): super().__init__() self.conv = DoubleConv(in_channels, out_channels) self.pool = nn.MaxPool2d(kernel_size=2, stride=2) def forward(self, x): x_conv = self.conv(x) x_pool = self.pool(x_conv) return x_conv, x_pool ". This returns both the convolution output (for skip connections) and the pooled output for the next layer.

In the decoder, we use upsampling with transposed convolution and concatenate with the corresponding encoder output before applying double convolution: "python class UpSample(nn.Module): def __init__(self, in_channels, out_channels): super().__init__() self.up = nn.ConvTranspose2d(in_channels, out_channels, kernel_size=2, stride=2) self.conv = DoubleConv(in_channels, out_channels) def forward(self, x1, x2): x1 = self.up(x1) x = torch.cat([x2, x1], dim=1) x = self.conv(x) return x ".

Finally, we define the U-Net model by stacking the downsampling and upsampling layers. The number of input channels (e.g., 3 for RGB) is passed to the first layer, and the number of output classes (1 for binary segmentation) determines the final convolution: "python class UNet(nn.Module): def __init__(self, in_channels, out_classes): super().__init__() self.down1 = DownSample(in_channels, 64) self.down2 = DownSample(64, 128) self.down3 = DownSample(128, 256) self.down4 = DownSample(256, 512) self.bottleneck = DoubleConv(512, 1024) self.up4 = UpSample(1024, 512) self.up3 = UpSample(512, 256) self.up2 = UpSample(256, 128) self.up1 = UpSample(128, 64) self.final_conv = nn.Conv2d(64, out_classes, kernel_size=1) def forward(self, x): d1, p1 = self.down1(x) d2, p2 = self.down2(p1) d3, p3 = self.down3(p2) d4, p4 = self.down4(p3) bn = self.bottleneck(p4) u4 = self.up4(bn, d4) u3 = self.up3(u4, d3) u2 = self.up2(u3, d2) u1 = self.up1(u2, d1) return self.final_conv(u1) ".

This completes the U-Net architecture and the data pipeline. You can now use the get_data_loader function to create batches and train the U-Net model for semantic segmentation of the "person" class in the dataset.

In our U-Net architecture, the first component is the downward convolution, defined as a double convolution block. We have four downsampling operations: the first goes from 64 to 128 channels, the second from 128 to 256, the third from 256 to 512, and the fourth from 512 to 1024, which completes the encoder part. The bottleneck layer is a double convolution taking 512 channels as input and outputting 1024 channels, defined as "self.bottleneck = DoubleConv(512, 1024)".

For the decoder, we perform upsampling in reverse order. We use four upsampling blocks, defined as "UpSample", which accept the output from the bottleneck and the corresponding encoder outputs (skip connections). The first upsample block goes from 1024 to 512 channels, the second from 512 to 256, the third from 256 to 128, and the fourth from 128 to 64 channels. The final output layer is a 1×1 convolution defined as "self.out = nn.Conv2d(64, num_classes, kernel_size=1)", producing a segmentation mask with the same number of channels as classes.

The forward pass is defined in a sequence of operations. We first pass the input through the downsampling blocks: "d1, p1 = self.down1(x); d2, p2 = self.down2(p1); d3, p3 = self.down3(p2); d4, p4 = self.down4(p3)". The bottleneck is computed as "b = self.bottleneck(p4)". Then we perform upsampling with concatenation of the corresponding encoder outputs: "up1 = self.up1(b, d4); up2 = self.up2(up1, d3); up3 = self.up3(up2, d2); up4 = self.up4(up3, d1)". Finally, the output is obtained using "out = self.out(up4); return out".

For the loss function, we implement Dice Loss to handle segmentation tasks: "class DiceLoss(nn.Module): def init(self, smooth=1e-6): super().init(); self.smooth = smooth; def forward(self, inputs, targets): inputs = inputs.view(-1); targets = targets.view(-1); intersection = (inputs * targets).sum(); dice = (2.0 * intersection + self.smooth) / (inputs.sum() + targets.sum() + self.smooth); return 1 - dice". To combine Dice Loss with Binary Cross Entropy, we define "class BCEWithDiceLoss(nn.Module): def init(self, smooth=1e-6): super().init(); self.bce = nn.BCEWithLogitsLoss(); self.dice = DiceLoss(smooth); def forward(self, inputs, targets): bce_loss = self.bce(inputs, targets); dice_loss = self.dice(torch.sigmoid(inputs), targets); return bce_loss + dice_loss / 2". This weighting ensures that Dice Loss has a stronger influence on segmentation accuracy.

The training loop is defined as "def train(model, dataloader, epochs=2, lr=0.001, save_path='U-Net_model.pt', load_path=None): device = torch.device('cuda' if torch.cuda.is_available() else 'cpu'); if load_path and os.path.exists(load_path): print('Loading model weights from load path'); model.load_state_dict(torch.load(load_path, map_location=device)); else: print('No checkpoint provided. Training from scratch.'); model.to(device); criterion = BCEWithDiceLoss(); optimizer = torch.optim.SGD(model.parameters(), lr=lr); for epoch in range(epochs): model.train(); epoch_loss = 0; for images, masks in dataloader: images, masks = images.to(device), masks.to(device); optimizer.zero_grad(); outputs = model(images); loss = criterion(outputs, masks); loss.backward(); optimizer.step(); epoch_loss += loss.item(); avg_loss = epoch_loss / len(dataloader); print(f'Epoch {epoch+1}/{epochs}, Loss: {avg_loss}, LR: {lr}'); if epoch > 0 and (epoch+1) % 10 == 0: torch.save(model.state_dict(), save_path); torch.save(model.state_dict(), save_path.replace('.pt', '_final.pt'))".

Once the model is trained, we perform inference using a simple predict function: "def predict(model_path, input_image_path, output_path='output.jpeg'): device = torch.device('cuda' if torch.cuda.is_available() else 'cpu'); model = UNet(3,1); model.load_state_dict(torch.load(model_path, map_location=device)); model.to(device).eval(); image = Image.open(input_image_path).convert('RGB'); transform = transforms.Compose([transforms.Resize((512,512)), transforms.ToTensor()]); img_tensor = transform(image).unsqueeze(0).to(device); with torch.no_grad(): pred = torch.sigmoid(model(img_tensor)); mask = (pred.squeeze().cpu().numpy() > 0.4).astype(np.uint8)*255; Image.fromarray(mask).save(output_path)". This produces a refined mask for the input image with minimal post-processing and high efficiency.

Finally, using a pre-trained U-Net model trained for many epochs, we can observe that the model accurately segments the “person” class in images, even with noisy and low-resolution datasets. The inference time is very low (under 0.2 seconds per image), making this end-to-end U-Net implementation efficient for practical use. The architecture, training loop, loss functions, and inference pipeline are now complete and fully functional.

**H) Mask-RCNN**

Hey everyone! Welcome back! So far, we have explored Fully Convolutional Networks (FCN) for semantic segmentation and U-Net, which is an improvement over FCN. We also saw how to train and test U-Net using PyTorch. Now, it is time to dive into instance segmentation, and for that, we will use Mask R-CNN. Mask R-CNN is a model designed specifically for instance segmentation. It is built on the foundation of Faster R-CNN and extends it by adding a segmentation mask branch. While Faster R-CNN predicts bounding boxes and class labels, Mask R-CNN predicts bounding boxes, class labels, and pixel-wise masks. The three outputs of Mask R-CNN are thus: the bounding box coordinates, the classification label, and the segmentation mask itself.

Mask R-CNN, like Faster R-CNN, is a two-stage object detector. The first stage uses a Region Proposal Network (RPN) to propose candidate regions where objects are likely to exist. The second stage processes each proposed region to predict bounding boxes, class labels, and masks. The two major improvements of Mask R-CNN over Faster R-CNN are: (1) the segmentation branch that predicts masks for each object and (2) ROI Align, which replaces ROI Pooling to avoid quantization issues and information loss when mapping regions of interest to fixed-size feature maps.

The architecture of Mask R-CNN starts with an input image (RGB or grayscale) that passes through a backbone network such as ResNet-50, ResNet-101, or VGG-16. For better multi-scale feature extraction, a Feature Pyramid Network (FPN) is used on top of the backbone. The FPN generates pyramid feature maps at different scales so that both small and large objects can be detected accurately. These feature maps are then sent to the RPN, which outputs region proposals. Each region proposal is processed individually using ROI Align, which converts regions of arbitrary sizes into a fixed-size feature map (e.g., 3×3×512) without losing spatial information due to quantization. The ROI Align uses bilinear interpolation on multiple sampling points within each cell to preserve fine details, unlike ROI Pooling, which suffers from information loss.

After ROI Align, the feature map is passed through fully connected layers for bounding box regression and classification. In parallel, the same feature map is passed through a mask branch—a small convolutional network—which outputs a pixel-wise mask for each object class. The mask branch typically ends with a 1×1 convolution layer producing a mask of size equal to the number of classes. This allows the model to output detailed object masks while also maintaining accurate bounding boxes and class predictions.

We can implement Mask R-CNN in PyTorch easily using torchvision. First, we import necessary libraries like numpy, cv2, matplotlib, PIL, and TorchVision models. Then, we can load a pre-trained Mask R-CNN model with ResNet-50 and FPN as the backbone: model = torchvision.models.detection.maskrcnn_resnet50_fpn(pretrained=True). We set the model to evaluation mode using model.eval() and prepare an input image by loading it with PIL.Image.open('person_walking.jpeg') and converting it to a tensor using transforms.ToTensor(). Next, we pass the image tensor through the model inside a torch.no_grad() block: output = model([image_tensor]). The model output contains boxes, labels, scores, and masks.

To filter predictions based on confidence, we can use a threshold, for example: threshold = 0.6 and select only predictions above this score. The masks can then be processed and overlaid on the original image using cv2.addWeighted(image_np, 1, color_mask, 0.5, 0), where color_mask = np.zeros_like(image_np) and we fill the mask channel according to predicted object masks. Finally, the resulting image can be saved using cv2.imwrite('output_image.png', result_image). By following these steps, we can perform instance segmentation on any image using Mask R-CNN in PyTorch, producing bounding boxes, class labels, and colored masks for detected objects.

In summary, Mask R-CNN extends Faster R-CNN by adding a segmentation branch and using ROI Align for precise pixel-level masks. It is capable of detecting multiple instances of objects in an image and producing high-quality masks. Using PyTorch, we can implement a pre-trained Mask R-CNN model with just a few lines of code, visualize results, and leverage its outputs for real-world applications such as object detection, instance segmentation, and scene understanding.

**I) Training Yolov11 Instance Segmentation**

Hey everyone! Welcome back. In this lecture, we are going to learn how to train our custom YOLOv11 instance segmentation model. YOLO, which stands for “You Only Look Once,” has multiple versions, and we will focus on YOLOv11 because it is faster in inference and has better accuracy compared to YOLOv12, even though YOLOv12 uses a transformer backbone for feature extraction. YOLOv11 provides enhanced feature extraction, optimized efficiency, speed, and versatility across tasks including classification, detection, and segmentation. Since we are only interested in segmentation, YOLOv11 is ideal for this project.

There are two main approaches to training YOLOv11: using the GitHub repository or the Command Line Interface (CLI). GitHub is useful when you want to download the source code, contribute to the project, modify code, or access the latest documentation, while the CLI allows quick training, inference, and model export without extensive setup. We will use the CLI approach because it allows efficient workflow and minimal code. To install YOLOv11 via CLI, you can run “!pip install ultralytics”, which installs all dependencies needed for YOLOv11 and other YOLO models.

Next, we need a dataset for instance segmentation. You can use Roboflow to create the dataset by creating a new project, selecting “Instance Segmentation,” and uploading your images. You can annotate the objects manually using the polygon tool or smart polygon tool. After annotation, split the dataset into train, validation, and test sets. Roboflow will generate a YOLOv11-ready ZIP file along with a “data.yaml” file that contains paths to the images and labels. Once the dataset is ready, upload it to Google Colab and unzip it using “!unzip dataset.zip -d dataset_folder”. The unzipped folder should contain train, valid, and test subfolders, each having images and labels. Make sure the paths in your “data.yaml” file match the folder structure.

To validate that your dataset is correct, you can check the number of images and labels in each folder by running “import os” and defining a function like “def count_files(folder): images = len(os.listdir(os.path.join(folder, 'images'))); labels = len(os.listdir(os.path.join(folder, 'labels'))); return images, labels”. You can then call “train_images, train_labels = count_files('dataset_folder/train')”, “valid_images, valid_labels = count_files('dataset_folder/valid')”, and “test_images, test_labels = count_files('dataset_folder/test')” to ensure consistency.

Once the dataset is verified, import YOLO from Ultralytics using “from ultralytics import YOLO”. You can choose from several pre-trained weights: nano, small, medium, large, and extra-large. For faster experimentation, we can use the nano model: “model = YOLO('yolov11n-seg.pt')”. To start training, pass the dataset path, number of epochs, image size, batch size, and other parameters. For example, you can write “results = model.train(data='dataset_folder/data.yaml', epochs=35, imgsz=640, batch=4, patience=5, project='YOLOv11_test', name='crack_detection', exist_ok=True, cache='disk', plots=True, val=True)”. Here, patience=5 stops training if no improvement is observed for five consecutive epochs, and cache='disk' ensures images are cached on disk.

During training, the model will automatically save weights, plots, and evaluation results in the project folder. You can visualize training loss, segmentation loss, classification loss, confusion matrix, and F1-score curves. If you want to resume training or use pre-trained weights, load them using “trained_model = YOLO('YOLOv11_test/crack_detection/weights/best.pt')”. For inference on new images or videos, you can run “results = trained_model.predict('test_image.jpg', imgsz=640, conf=0.5)” and display the predictions using “results.show()”.

This approach allows you to train, validate, and infer using a custom YOLOv11 instance segmentation model with minimal Python code. In the next lecture, we will focus on evaluating the trained model on new images or videos to test real-world performance and analyze segmentation accuracy.

**J) Testing Yolov11 Instance Segmentation**

Hey everyone! Welcome back. In this lecture, we are going to test our YOLOv11 instance segmentation model trained on our custom dataset. Testing the model means performing inference or predicting on new data. According to the YOLO documentation under the predict section, you can get multiple predictions from the model. To begin, we load the trained model. Instead of using a pre-trained YOLOv11n-seg or other YOLOv11 model, we will load our custom model by passing the path to the trained weights. YOLO supports predictions on images, videos, and batches of images. The model returns an iterator, where results.boxes provides bounding boxes, results.masks provides object masks, results.keypoints provides pose outputs, results.probs provides classification outputs, and results.obb provides oriented bounding boxes. You can visualize results using results.show() or save the output image locally.

In Google Colab, we can connect to a Tesla T4 GPU instance and unzip the trained model. After installing Ultralytics with "!pip install ultralytics", we import necessary libraries using "import os, cv2". Then we unzip the model weights with "!unzip yolov11_test1.zip -d train". Inside the train folder, the model weights are present in the weights folder, where last.pt is the final epoch and best.pt is the optimal model with balanced box, classification, object, and segmentation loss. We will use this best.pt for inference. To load the model, we import YOLO with "from ultralytics import YOLO" and initialize it with "model = YOLO('content/train/weights/best.pt')". The model will automatically recognize that it is a segmentation model.

For inference on a single image, we specify the image path. Suppose our test image is "stone_texture2.jpeg"; we can perform inference by passing the model and image path like "results = model.predict('stone_texture2.jpeg')". We then iterate over the results, though a single image returns only one result. Visualization can be done using "results.show()", and we can save the output image by extracting the file name using "fname, ext = os.path.splitext('stone_texture2.jpeg')" and saving as "cv2.imwrite(fname + '_output.jpg', result_image)". In Colab, inference speed per image is around 16.5 ms for preprocessing, 52.6 ms for inference, and 56.3 ms for post-processing, with an input dimension of 608x640 in RGB format. The model performs well even on images containing cracks not present in the training set.

For batch inference, we can create a list of image paths such as "batch = ['stone_texture1.jpeg', 'stone_texture2.jpeg']" and pass it to the model like "results = model.predict(batch)". Iterating with enumerate allows us to process each result separately, save outputs with unique file names, and track performance on multiple images. For example, we can use "for idx, result in enumerate(results): cv2.imwrite(f'image_{idx}_output.jpg', result.imgs[0])". This approach works well when receiving frames from a camera stream.

If we have images as numpy arrays, such as frames from a video, we can read an image using OpenCV "img = cv2.imread('stone_texture2.jpeg')" and pass it directly to the model with "results = model.predict(img)". YOLOv11 segmentation models accept numpy arrays, making them flexible for real-time camera streams.

To extract masks from the results, we import necessary libraries "import torch, matplotlib.pyplot as plt". If result.masks is present, it contains detected objects. The mask data is complex and can be accessed using "mask = result.masks.data", while bounding boxes are accessed with "boxes = result.boxes.data". Suppose we want to extract a specific class, e.g., cracks. We can filter indices where "torch.where(result.boxes.cls == 0)" to get only those objects. If no mask is found, checking "len(mask) == 0" allows us to handle empty outputs. For visualization, we scale the mask from 0-1 to 0-255 and move it to CPU for matplotlib using "mask = mask.cpu().numpy()". Finally, we display the mask with "plt.imshow(mask, cmap='gray')", producing a binary black-and-white output.

This workflow demonstrates how to perform inference on single images, batches of images, and numpy arrays using a custom-trained YOLOv11 instance segmentation model. It also shows how to extract and visualize semantic segmentation masks from instance segmentation outputs. With this, we know how to train, test, and perform inference on our YOLOv11 instance segmentation model effectively.
